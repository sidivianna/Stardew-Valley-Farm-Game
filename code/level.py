...
class Level:
    ...
    def setup(self):
        ...
        # water
        water_frames = import_folder('../Stardew Valley Game/graphics/water')
...
class CameraGroup(pygame.sprite.Group):
    ...
    def custom_draw(self, player):
        ...
        for layer in LAYERS.values():
            for sprite in sorted(self.sprites(), key = lambda sprite: sprite.rect.centery): # usado para criar efeito de sobreposição da imagem 
                if sprite.z == layer:
                    offset_rect = sprite.rect.copy()
                    offset_rect.center -= self.offset
                    self.display_surface.blit(sprite.image, offset_rect)

                    # anaytics (usado para entender o alvo do moveimento das ferramentas)
                    '''if sprite == player:
